...
class Detect(object):
    ...
    def mycustum1(self, df):
        # 1. 前7根趨勢為正
        # 2. 第8根上漲
        # 3. 第10根下跌
        # 4. 第8根長度在前50根中PR值大於等於55
        # 5. 第9根長度在前50根中PR值小於等於45
        ...
        cond4 = (df['realbody_per'].iloc[-3] >= 55)
        cond5 = (df['realbody_per'].iloc[-2] <= 45)
        ...
        if cond1 and cond2 and cond3 and cond4 and cond5 and cond6 and cond7 and cond8:
            ...
            return False
        #建立Dark Cloud
        #def mydarkCloud(self, df):
        #1. 前7根趨勢為正
        #2. 第8根下跌
        #3. 第9根開盤小於第7根body中央
        #cond1 = (df['trend7'].iloc[-4] > 0)
        #cond2 = (df['direction'].iloc[-3] < 0)
        #cond3 = (df['open'].iloc[-2] <= (df['open'].iloc[-4] + df['realbody'].iloc[-4] * (1/2)))
    ...
    def process(self):
        ...
        if 60 <= (self.data['date'].iloc[1] - self.data['date'].iloc[0]).seconds < 1800 :
            ...
            self.timeScale = '1W'
        # 新增欲偵測的交易訊號
        self.data['mycustum1'] = 0
        self.data['mycustum2'] = 0
    ...
    def signal(self):
        ...
        for idx in tqdm(self.data.index):
            ...
            if start_idx >= 0:
                ...
                if self.mycustum1(df):
                    self.data.loc[end_idx, 'mycustum1'] = 1
                elif self.mycustum2(df):
                    self.data.loc[end_idx, 'mycustum2'] = 1
                ...
                #elif self.mydarkCloud(df):
                #    self.data.loc[end_idx, 'mydarkCloud'] = 1
                else:
    ...
    def result(self):
        ...
        print('mycustum1: {}'.format(self.data.loc[self.data['mycustum1'] == 1, 'mycustum1'].shape[0]))                   
        print('mycustum2: {}'.format(self.data.loc[self.data['mycustum2'] == 1, 'mycustum2'].shape[0]))
